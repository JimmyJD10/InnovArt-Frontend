...
export default function MisProductos() {
  ...
  const fetchProductos = async () => {
    ...
    const res = await axios.get('http://172.31.15.139:3001/api/products', {
  ...
  const handleCrear = async () => {
    const token = localStorage.getItem('token')
    await axios.post('http://172.31.15.139:3001/api/products', {
  ...
  const handleActualizar = async () => {
    const token = localStorage.getItem('token')
    await axios.put(`http://172.31.15.139:3001/api/products/${editId}`, {
      titulo, descripcion, precio: parseFloat(precio)
    }, { headers: { Authorization: `Bearer ${token}` } })
    setEditId(null)
    setTitulo('')
  ...
  const handleEliminar = async (id: number) => {
    const token = localStorage.getItem('token')
    await axios.delete(`http://172.31.15.139:3001/api/products/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    fetchProductos()
  }
