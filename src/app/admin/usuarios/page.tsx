...
export default function AdminUsuariosPage() {
  ...
  const fetchUsuarios = async () => {
    ...
    const res = await axios.get('http://172.31.15.139:3001/api/users', {
  ...
  const handleEliminar = async (id: number) => {
    ...
    const token = localStorage.getItem('token')
    await axios.delete(`http://172.31.15.139:3001/api/users/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    fetchUsuarios()
  }
  ...
  const handleRol = async (id: number, rol: string) => {
    const token = localStorage.getItem('token')
    await axios.put(`http://172.31.15.139:3001/api/users/${id}`, { rol }, {
      headers: { Authorization: `Bearer ${token}` }
    })
    fetchUsuarios()
  }
